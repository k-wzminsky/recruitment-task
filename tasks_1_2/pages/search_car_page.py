...
class SearchCarPage(base_page.BasePage):
    class Misc:
        ...
        TABLE_CELL_BY_INDEX = FormattedLocator(
            by.By.XPATH,
            '//tbody/tr/th[text()="{table_index}"]/following-sibling::td[{header_index}]',
        )
        TABLE_CELL_BY_TD = FormattedLocator(
            by.By.XPATH, '//table/tbody/tr[td[text()="{td_text}"]]'
        )

    class Buttons:
        SEARCH = (by.By.CSS_SELECTOR, "button[type=submit]")
        TABLE_RENT_BUTTON_BY_TD = FormattedLocator(
            by.By.XPATH, '//table/tbody/tr[td[text()="{td_text}"]]//a'
        )
